...
class StatisticalService
{
    ...
    public static function OrderCompleteMoneyYesterdayTodayTotal($params = [])
    {
        ...
        $total_count = Db::name('Order')->where($where)->where('status','<>',1)->sum('total_price');
        ...
        $yesterday_count = Db::name('Order')->where($where)->where('status','<>',1)->sum('total_price');
        ...
        $today_count = Db::name('Order')->where($where)->where('status','<>',1)->sum('total_price');
