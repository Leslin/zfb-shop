...
class OrderService
{
    ...
    public static function Notify($params = [])
    {
        ...
        // 获取订单信息
        $where = ['order_no'=>$ret['data']['out_order_no'], 'is_delete_time'=>0, 'user_is_delete_time'=>0];
        $order = Db::name('Order')->where($where)->find();
        ...
        $pay_params = [
            ...
            'payment'   => $payment[0],
            'pay'       => [
                'trade_no'      => $ret['data']['out_order_no'],
                'subject'       => '线上资金授权冻结',
                'buyer_user'    => $ret['data']['payer_logon_id'],
                'pay_price'     => $ret['data']['credit_amount'],
                'return_json'   => json_encode($ret),
                'auth_no'       => $ret['data']['auth_no']
            ],
        ];
        return self::OrderPayHandle($pay_params);
    }
    ...
    public static function OrderPayHandle($params = [])
    {
        ...
        $upd_data = array(
            ...
            'pay_time'      => time(),
            'upd_time'      => time(),
            'return_json'   => $params['pay']['return_json'],
            'auth_no'       => $params['pay']['auth_no']
        );
        if(Db::name('Order')->where(['id'=>$params['order']['id']])->update($upd_data))
        {
            ...
            if(self::OrderHistoryAdd($params['order']['id'], 2, $params['order']['status'], '支付', 0, '系统'))
            {
                ...
                //增加子订单列表
                $order_detail = Db::name('order_detail')->where('order_id',$params['order']['id'])->find(); //详情
                $order_info = Db::name('Order')->where(['id'=>$params['order']['id']])->find();
                $sub_order = Db::name('order_sub')->where('out_order_no',$order_info['order_no'])->find();
                if (!empty($order_detail) && empty($sub_order)) {
                    $spec = json_decode($order_detail['spec'],1);  //取出规格
                    $days = $spec[0]['value'] ;  //租用天数
                    //生成子订单
                    for ($i=1; $i <= $days; $i++) {
                       if ($i == 1) {   //第一天
                           $repayment_time = date('Y-m-d',strtotime('+ '.($i-1).' days')); //当前，后面有2天没有
                       }else{
                           $repayment_time = date('Y-m-d',strtotime('+ '.($i+1).' days'));
                       }
                       
                       $sub_order = [
                           'out_order_no' => $order_info['order_no'],
                           'sub_out_order_no' => $order_info['order_no'].'-'.$i,
                           'auth_no'     =>$order_info['auth_no'],
                           'amount'      => $order_detail['price'],
                           'total_fee'   => $order_info['total_price'],
                           'times'       => date('Y-m-d').'~'.date('Y-m-d',strtotime('+ '.$days.' days')),
                           'term'        => $days,
                           'repayment_time' => $repayment_time,
                           'nums'          => $i,
                       ];
                       Db::name('order_sub')->insert($sub_order);
                    }
                }
    ...
    public static function OrderList($params = [])
    {
        ...
        if(!empty($data))
        {
            ...
            foreach($data as &$v)
            {
                ...
                //账单信息
                $v['sub_order'] = Db::name('order_sub')->where('out_order_no',$v['order_no'])->count(); //账单总数
                $v['re_sub_order_cnt'] = Db::name('order_sub')->where('out_order_no',$v['order_no'])->where('status',1)->count();
                $v['re_sub_order_sum'] = Db::name('order_sub')->where('out_order_no',$v['order_no'])->where('status',1)->sum('amount');

                $v['no_sub_order_cnt'] = Db::name('order_sub')->where('out_order_no',$v['order_no'])->where('status',0)->count();
                $v['no_sub_order_sum'] = Db::name('order_sub')->where('out_order_no',$v['order_no'])->where('status',0)->sum('amount');
